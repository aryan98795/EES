...
const Navbar = () => {
  ...
  return (
    <nav
      className={`sticky top-0 z-50 border-b ${darkMode ? "bg-[#0f172a] text-white border-slate-800" : "bg-white text-slate-800 border-slate-200/50"}`}
    >
        ...
        <div className="flex h-20 items-center justify-between">
          <Link to="/" className="flex items-center gap-2">
            <div className="bg-blue-600 p-2 rounded-lg">
              <img
                src="/EES_logo.jpeg"
                className="h-7 w-7 rounded-md invert brightness-200"
                alt="Logo"
              />
            </div>
            ...
            <div className="hidden sm:flex items-center gap-4">
              {!isLoggedIn ? (
                <>
                  ...
                  <Link
                    to="/login"
                    className="px-4 py-2 rounded-lg border border-blue-600 text-blue-600 hover:bg-blue-50 transition-all"
                  >
                    Sign In
                  </Link>
                </>
                ...
                <Link
                  to="/logout"
                  className="px-4 py-2 bg-red-500 text-white rounded-lg transition-colors duration-300 hover:bg-red-600 active:bg-red-700"
                >
                  Logout
                </Link>
              )}
                ...
                <div className="flex flex-col gap-3">
                  {!isLoggedIn ? (
                    <>
                      <Link
                        to="/login"
                        className="w-full py-3 text-center rounded-xl bg-blue-600 text-white"
                      >
                        Get Started
                      </Link>
                      <Link
                        to="/signup"
                        className="w-full py-3 text-center rounded-xl border border-blue-600 text-blue-600"
                      >
                        Sign Up
                      </Link>
                    </>
                    ...
                    <Link
                      to="/logout"
                      className="w-full py-3 text-center rounded-xl bg-red-500 text-white"
                    >
                      Logout
                    </Link>
                  )}
